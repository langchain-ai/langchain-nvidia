...
def pytest_addoption(parser: pytest.Parser) -> None:
    parser.addoption("--chat-model-id", action="store",
                     help="Run tests for a specific chat model",)
    parser.addoption("--all-models", action="store_true",
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    ...
    available_models = ChatNVIDIA().mode(**mode).get_available_models(list_all=True,
                                                                      **mode)
